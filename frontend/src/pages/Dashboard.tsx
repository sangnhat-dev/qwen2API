  ...
  useEffect(() => {
    const fetchStatus = async () => {
      try {
        ...
        if (!errOnce) {
          toast.error("Failed to fetch status. Please check your current session key in System Settings.")
          setErrOnce(true)
        }
      }
    }
  ...
  return (
    <div className="space-y-8 max-w-5xl relative">
      <div className="relative z-10">
        ...
        <h2 className="text-3xl font-extrabold tracking-tight bg-gradient-to-r from-foreground to-foreground/60 bg-clip-text text-transparent">System Status</h2>
        <p className="text-muted-foreground mt-2 text-lg">Real-time concurrency monitoring and Qwen account pool overview (auto-refreshes every 3 seconds).</p>
      </div>
      ...
      <div className="grid gap-6 md:grid-cols-2 relative z-10">
        <StatCard icon={<Flame className="h-5 w-5 text-rose-400" />} title="Chat_ID Warm Pool" value={String(pool?.total_cached ?? 0)} accent="rose" sub={pool ? `Target per account ${pool.target_per_account} · TTL ${Math.round((pool.ttl_seconds || 0) / 60)} min` : "Not enabled"} />
            ...
            <table className="w-full text-sm">
              <thead className="bg-muted/20 text-xs uppercase text-muted-foreground">
                <tr>
                  ...
                  <th className="text-right px-4 py-3 font-semibold">Warm chat_id</th>
      ...
      <div className="rounded-2xl border border-border/50 bg-card/30 backdrop-blur-xl shadow-2xl relative overflow-hidden">
        ...
        <div className="flex flex-col space-y-2 p-8 border-b border-border/50 bg-muted/10 relative z-10">
          ...
          <p className="text-base text-muted-foreground ml-5">Compatible with mainstream AI protocols. Access by default without authentication, or via API Key.</p>
        </div>
